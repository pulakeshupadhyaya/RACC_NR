...
def load_FTR_data(label,i):
    ...
    if(label == 'latex'):
        j = 3
    folder_name = "4095_noisy_randomized_0p016/testing/"
...
def load_CM_data(label, i):
    ...
    out_folder_name = "4095_noisy_randomized_0p016/testing/"
...
def DNN_model_load(pred):
    ...
    if (pred == 0):
        ...
        model.load_weights('auto_weights_html_4_0p014.h5')
    if (pred == 1):
         # adapt this if using `channels_first` image data format
        ...
        model.load_weights('auto_weights_jpeg_11_0p014.h5')
    ...
    if(pred == 2):
        ...
        model.load_weights('auto_weights_pdf_8_0p014.h5')
    if(pred == 3):
        x = Conv2D(100, (8,1), activation='relu')(input_img)
        x = Conv2D(200, (3, 1), activation='relu')(x)
        x = Conv2D(300, (3, 1), activation='relu')(x)
        ...
        model.load_weights('auto_weights_latex_6_0p014.h5')
    gc.collect()
...
def FTR(label,i):
    ...
    model = load_model('4095_0p014_my_model_9layer_0drop.h5')
    model.load_weights('4095_0p014_my_model_weights_9layer_0drop.h5')
...
def store_DNN_values(label,i):
    ...
    del x_test
    del y_test    
    filename = open("results_0p014/"+label+"/0p016/"+str(i)+".csv",'w')
